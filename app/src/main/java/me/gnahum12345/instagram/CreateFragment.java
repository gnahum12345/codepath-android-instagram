...
public class CreateFragment extends Fragment implements ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    private String mCameraId; // ID of the current Camera
    private AutoFitTextureView mTextureView; // for camera preview
    private CameraCaptureSession mCaptureSession; // camera previe
    private CameraDevice mCameraDevice; // a reference to the opened camera device.
    private Size mPreviewSize; // size of camera preview. (android.util.size);.
    ...
    private HandlerThread mBackgroundThread; // An additional thread for running tasks in the background
    ...
    private CaptureRequest mPreviewRequest;  // generate by builder.

    private int mState = STATE_PREVIEW; //current state;
